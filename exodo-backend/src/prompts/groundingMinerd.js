...
function buildSystemPrompt(opts) {
  ...
  if (o.lite) {
    ...
    const liteIdentity = [
      ...
      'CITACIÓN OBLIGATORIA en temas de hechos históricos, datos empíricos, ciencia, medicina, leyes o biografías: incrusta DENTRO de la frase, justo tras el dato, un enlace Markdown `[Nombre Corto de la Página](https://...)` (1-3 palabras, sin prefijos "consultar:"/"fuente:").',
      'Los enlaces van SOLO incrustados en la frase del dato: NUNCA agrupados al final, en línea aparte ni en sección de fuentes (`### Fuentes` PROHIBIDA: la app extrae los enlaces y muestra su cápsula de Sources).',
      'SOLO fuentes acreditadas: archivos nacionales, academias de historia, UNESCO, Britannica, Nature, PubMed, portales oficiales. En saludos, charla casual, redacción creativa o código: CERO fuentes.',
...
function buildIdentitySection(plan, locale, messageLang) {
  ...
  return [
    ...
    '  * Citación en línea (justo en el dato): Coloca la referencia como un enlace Markdown INCURSTADO DENTRO de la frase donde das la información, justo después del dato, con texto visible = nombre corto de la página o institución (1-3 palabras, SIN prefijos como "consultar:" o "fuente:"). Ejemplo exacto: "En 1697 [AGN](https://agn.gob.do), España cede la parte occidental de la isla a Francia...". La app pinta ese enlace como una barra discreta al lado del texto. PROHIBIDO agrupar los enlaces al final del párrafo, en una línea aparte o al cierre de la respuesta: cada barra va incrustada en la frase del dato que respalda.',
